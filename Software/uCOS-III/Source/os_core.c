...
//   if(OSPrioHighRdy == 3 && ((scpSystemCeiling->ceiling != -1 && OSTCBHighRdyPtr->period > scpSystemCeiling->ceiling) || scpSystemCeiling->ceilingCreator->currentHolder != OSTCBHighRdyPtr)){
 //    CPU_INT_EN(); 
//     return;
//   }
...
void  OS_RdyListInsertHead (OS_TCB  *p_tcb)
{
    ...
    if(p_tcb->Prio==3){
        ...
        //printf("Inserting into minHeap %s\n", p_tcb->NamePtr);
        p_rdy_list->HeadPtr = insertMinHeap(p_tcb, p_rdy_list->HeadPtr);
    }
    else if (p_rdy_list->NbrEntries == (OS_OBJ_QTY)0) {          /* CASE 0: Insert when there are no entries               */
...
void  OS_RdyListRemove (OS_TCB *p_tcb)
{
    ...
    if(p_tcb->Prio==3){
        //printf("Deleting from minHeap %s\n", p_tcb->NamePtr);
        p_rdy_list->HeadPtr = deleteHeapNode(p_rdy_list->HeadPtr, p_tcb->deadline);
        if(p_rdy_list->HeadPtr == NULL){
            p_rdy_list->NbrEntries = (OS_OBJ_QTY)0; 
    ...
    else if (p_tcb1 == (OS_TCB *)0) {                            /* Was the OS_TCB to remove was at the head?              */
