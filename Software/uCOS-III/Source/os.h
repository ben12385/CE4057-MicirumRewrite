...
struct os_tcb {
    ...
    OS_TCB              *leftChildHeap;
    OS_TCB              *rightChildHeap;
    int                 deadline;
    int                 numberOfChild;
    int                 period;
};
...
OS_EXT            CPU_TS                 OSTickTaskPeriodicTimeMax;
...
int canRun(struct node *avlNode, struct rbNode *rbNodeToBlock);
...
//Deletion of recursive task
void  OSTaskDelPeriodic (struct node *avlNode);

//Synchronous release of recursive tasks
extern int flag;
extern int baseTick;
...
OS_TCB* findNode(OS_TCB* root, int key);
